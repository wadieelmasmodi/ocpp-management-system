...
export const getOverview = async (req, res) => {
    try {
        ...
        const [totalStations, totalChargers, onlineChargers, offlineChargers, activeTransactions, activeRfidSessions, todayEnergy, todayRfidEnergy, totalEnergy, totalRfidSessions,] = await Promise.all([
        ...
        const connectorStatusDistribution = {};
...
export const getDistribution = async (req, res) => {
    try {
        ...
        const distribution = connectors.reduce((acc, connector) => {
            const status = connector.status;
            if (!acc[status]) {
                acc[status] = { count: 0, connectors: [] };
            }
            acc[status].count++;
 ...
 */
export const getChargersStatus = async (req, res) => {
    try {
